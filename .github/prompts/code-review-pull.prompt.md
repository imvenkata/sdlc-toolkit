...
Fetch MR context through GitLab MCP, list changed files before fetching diffs, batch diff retrieval 3-5 files at a time, and ask before posting anything back to GitLab. Use stable finding IDs and the shared output format from `.github/skills/code-review/SKILL.md`.

After the report, stay in follow-up mode for same-session actions such as posting the summary, drafting selected findings, posting inline comments, replying to known threads, resolving known threads, approving, or unapproving. Preview every GitLab write action and wait for explicit confirmation before executing.
